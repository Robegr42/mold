...
def main():
    ...
    meaningful_changes = [
        f for f in changed_files if not f.startswith(".gemini/") and f != journal_path
    ]
    ...
    if not os.path.exists(journal_path):
        print(
            "Error: Updated journal required. Please read the latest entries in journal/ to understand context,"
        )
        print(
            "then use: python3 .gemini/scripts/journal.py 'one line summary of changes'"
        )
        return 1
    ...
    if not lines:
        ...
        print(
            "Please use: python3 .gemini/scripts/journal.py 'one line summary of changes'"
        )
        return 1
    ...
    if last_entry_time < max_mtime:
        ...
        print(
            f"Please read {journal_path} to catch up, then add a summary of your latest work using:"
        )
