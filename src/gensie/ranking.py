...
A report is treated as the baseline for its model when its ``config.pipeline``
matches ``baseline_pipeline`` (default ``"baseline"`` — the name used by the
...
def compute_ranking(
    reports: List[Dict[str, Any]], baseline_pipeline: str = "baseline"
) -> Dict[str, Any]:
    ...
    for model in models:
        ...
        if len(base) > 1:
            warnings.append(
                f"model '{model}' has {len(base)} baseline reports — using the highest F1"
            )
        baselines[model] = max(b["f1"] for b in base)
    ...
    for model in ranked_models:
        ...
        for e in entries:
            if e["model"] != model or e["pipeline"] == baseline_pipeline:
                continue
