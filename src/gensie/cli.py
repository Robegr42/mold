...
def eval(
    ...
    """
    Enhanced evaluation: tracks tokens, time, and specific field errors.
    """
    evaluator = Evaluator()
    ...
    results_table = Table(title=f"Detailed Evaluation (Pipeline: {pipeline})")
    results_table.add_column("Task ID", style="cyan")
    results_table.add_column("Model", style="blue")
    results_table.add_column("TPS", justify="right")
    results_table.add_column("Tokens", justify="right", style="magenta")
    results_table.add_column("Time (s)", justify="right", style="yellow")
    ...
    with httpx.Client(timeout=60.0) as client:
        ...
        for file_path in track(json_files, description="Processing tasks..."):
            ...
            try:
                ...
                # Extract tokens
                tokens = system_output.pop("_tokens", 0)
                ...
                errors = evaluator.get_incorrect_fields(
                    task.output, system_output, task.target_schema
                )
            ...
            if task:
                ...
                results_table.add_row(
                    ...
                    str(tokens),
                    f"{duration:.2f}",
                    error_summary or "[green]None[/green]",
                )
                individual_results.append(
                    {
                        "task_id": task.id,
                        ...
                        "time_s": duration,
                        "errors": errors,
                        "status": "PASS" if not any("Call Failed" in err for err in errors) else "FAIL",
                    }
                )
    ...
    if output:
        ...
        with open(output, "w", encoding="utf-8") as f:
            json.dump(report, f, indent=2, ensure_ascii=False)
        console.print(f"\n[bold blue]Detailed report saved to {output}[/bold blue]")
