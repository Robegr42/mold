...
# Run evaluation for synthetic-anchor
echo "Evaluating pipeline: synthetic-anchor..."
uv run gensie eval2 \
    --data "$DATASET" \
    --url "$URL" \
    --pipeline "synthetic-anchor" \
    --model "$MODEL" \
    --output "$OUTPUT_DIR/synthetic_anchor_report.json"

echo "------------------------------------------------"
echo "Experiment complete. Report saved to $OUTPUT_DIR/synthetic_anchor_report.json"

# Verification
if [ -f "$OUTPUT_DIR/synthetic_anchor_report.json" ]; then
    echo "Verification SUCCESS: Result file created."
else
    echo "Verification FAILURE: Result file missing."
    exit 1
fi
